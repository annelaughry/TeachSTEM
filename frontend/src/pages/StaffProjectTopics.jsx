...
export default function StaffProjectTopics() {
  ...
  useEffect(() => {
    api.get('program-staff/project-topics/').then(r => {
      setHistory(r.data)
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      if (id) {
        const { data } = await api.put(`program-staff/project-topics/${id}/`, payload)
        setHistory(prev => prev.map(e => e.id === data.id ? data : e))
        setExpanded(data.id)
      } else {
        const { data } = await api.post('program-staff/project-topics/', payload)
  ...
  const submitForReview = async () => {
    ...
    try {
      ...
      const targetId = id
        ? (await api.put(`program-staff/project-topics/${id}/`, payload)).data.id
        : (await api.post('program-staff/project-topics/', payload)).data.id
      const { data: submitted } = await api.post(`program-staff/project-topics/${targetId}/submit/`)
  ...
  const submitExisting = async (id) => {
    setError(null)
    try {
      const { data } = await api.post(`program-staff/project-topics/${id}/submit/`)
      ...
      <div className="container" style={{ maxWidth: 740, paddingBottom: '3rem' }}>
        ...
        <div className="card" style={{ borderTop: '4px solid var(--yellow)', marginBottom: '2rem' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label className="form-label">What will you build, investigate, or create?</label>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label className="form-label">Background concepts needed before starting this project</label>
            <textarea className="form-input" rows={5} value={form.background_concepts} onChange={e => set('background_concepts', e.target.value)} />
            <p className="text-muted text-sm" style={{ marginTop: '0.3rem' }}>Describe the prerequisite knowledge and skills needed.</p>
          </div>
          ...
          <div style={{ background: 'var(--yellow-light)', border: '1px solid var(--yellow-light)', borderRadius: 8, padding: '0.85rem 1.1rem', marginBottom: '1.25rem', fontSize: '0.88rem', color: 'var(--yellow-dark)' }}>
