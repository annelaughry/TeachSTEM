...
import api from '../api'
import { SECTIONS } from '../data/tstemSurvey'
...
export default function TStemSurvey() {
  ...
  useEffect(() => {
    api.get('teach-stem/tstem-survey/')
      .then(r => {
  ...
  const save = async (opts = {}) => {
    ...
    setError('')
    try {
      await api.post('teach-stem/tstem-survey/', {
        responses,
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      await api.post('teach-stem/tstem-survey/', { responses, completed: true })
      setDone(true)
    } catch {
      ...
      <div className="page">
        <div className="container" style={{ maxWidth: 680, marginTop: 80, paddingBottom: '3rem' }}>
          <div className="card" style={{ textAlign: 'center', padding: '2.5rem 2rem' }}>
            ...
            <p style={{ marginBottom: '1.5rem' }}>Thank you for completing the T-STEM Science Teacher Survey. Your responses have been recorded.</p>
            <Link to="/teach-stem" className="btn btn--primary">Back to Dashboard</Link>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="page">
      <div className="hero" style={{ marginTop: 'var(--nav-h)', background: 'linear-gradient(135deg, var(--teal) 0%, var(--teal-dark) 100%)', paddingBottom: '1.5rem' }}>
        <h1>T-STEM Science Teacher Survey</h1>
      ...
      <div className="container" style={{ maxWidth: 780, paddingTop: '2rem', paddingBottom: '4rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            <Link to="/teach-stem" style={{ fontSize: '0.85rem', color: 'var(--text-muted)', textDecoration: 'underline' }}>Exit to Dashboard</Link>
            {!isLast && (
