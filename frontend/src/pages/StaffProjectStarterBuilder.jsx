...
const STATUS_LABELS = {
  ...
  submitted: { label: 'Submitted for Review', color: 'var(--yellow-dark)' },
...
export default function StaffProjectStarterBuilder() {
  ...
  useEffect(() => {
    api.get('program-staff/project-starters/').then(r => {
      setHistory(r.data)
    }).catch(() => {})
  }, [])
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      if (form.id) {
        const { data } = await api.put(`program-staff/project-starters/${form.id}/`, payload)
        setHistory(prev => prev.map(e => e.id === data.id ? data : e))
      } else {
        const { data } = await api.post('program-staff/project-starters/', payload)
  ...
  const submitForReview = async () => {
    ...
    try {
      ...
      let id = form.id
      if (id) {
        const { data } = await api.put(`program-staff/project-starters/${id}/`, payload)
        setHistory(prev => prev.map(e => e.id === data.id ? data : e))
      } else {
        const { data: created } = await api.post('program-staff/project-starters/', payload)
        id = created.id
        setHistory(prev => [created, ...prev])
      }
      const { data: submitted } = await api.post(`program-staff/project-starters/${id}/submit/`)
  ...
  const submitExisting = async (id) => {
    setError(null)
    try {
      const { data } = await api.post(`program-staff/project-starters/${id}/submit/`)
  ...
  return (
    <div className="page">
      ...
      <div className="container" style={{ maxWidth: 780, paddingBottom: '3rem' }}>
        ...
        {/* Form */}
        <div className="card" style={{ borderTop: '4px solid var(--yellow)', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
            <div>
              <h3 style={{ color: 'var(--yellow-dark)', marginBottom: '0.25rem' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <textarea
              className="form-input"
              rows={4}
              placeholder="Briefly describe what you will build, investigate, or create."
              value={form.overview}
          ...
          <p className="text-muted text-sm" style={{ marginBottom: '0.75rem' }}>
            The skills you'll practice — e.g. "Coding with Python", "Circuit Design".
          </p>
            ...
            {form.steps.map((step, si) => (
              <div key={si} className="card" style={{ background: '#fafafa', marginBottom: '0.75rem', padding: '1rem' }}>
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.6rem' }}>
                  <div style={{ fontWeight: 800, color: 'var(--yellow-dark)', flexShrink: 0 }}>{si + 1}.</div>
                  <input
                    type="text"
                    className="form-input"
            ...
            {form.tips.map((tip, i) => (
              <div key={i} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.5rem' }}>
                ...
                <input
                  ...
                  className="form-input"
                  style={{ flex: 1 }}
                  placeholder="A short piece of advice"
                  value={tip}
          ...
          <div style={{ background: 'var(--yellow-light)', border: '1px solid var(--yellow-light)', borderRadius: 8, padding: '0.85rem 1.1rem', marginTop: '1.75rem', marginBottom: '1.25rem', fontSize: '0.88rem', color: 'var(--yellow-dark)' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            ...
            <button type="button" className="btn btn--yellow" onClick={submitForReview} disabled={saving || submitting}>
                    ...
                    <div style={{ padding: '0 1.25rem 1.25rem', borderTop: '1px solid var(--border)' }}>
                      ...
                      {entry.status === 'draft' && (
                        <div style={{ display: 'flex', gap: '0.6rem', marginTop: '1rem' }}>
                          ...
                          <button type="button" className="btn btn--yellow btn--sm" onClick={() => submitExisting(entry.id)}>
                            Submit for Review
                          </button>
                        </div>
                      )}
